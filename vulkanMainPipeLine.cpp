...
void framebufferResizeCallback(GLFWwindow* window, int width, int height) {
	auto application = reinterpret_cast<app*>(glfwGetWindowUserPointer(window));
	application->set_framebufferResized(true);
}

void app::mainLoop() {
	while (!glfwWindowShouldClose(window)) {
		glfwPollEvents();
		drawFrame(window, device, graphicsQueue, presentQueue, swapChain, imageAvailableSemaphores, renderFinishedSemaphores, commandBuffers, currentFrame, inFlightFences, 
			imagesInFlight, swapChainImages, physicalDevice, surface, swapChainImageFormat, swapChainExtent, swapChainImageViews, renderPass,
...
void app::cleanUp() {
	...
	cleanupSwapChain(device, swapChainFramebuffers, commandPool, commandBuffers,
		graphicsPipeline, pipelineLayout, renderPass, swapChainImageViews, swapChain,
		swapChainImages, uniformBuffers, uniformBuffersMemory, descriptorPool, 
...
void app::initVulkan() {
	...
	createLogicalDevice(surface, physicalDevice, device, graphicsQueue, presentQueue);
	createSwapChain(device, physicalDevice, window, surface, swapChain, swapChainImages, swapChainImageFormat, swapChainExtent);
	createImageViews(device, swapChainImages, swapChainImageFormat, swapChainImageViews);
	createRenderPass(device, physicalDevice, swapChainImageFormat, renderPass);
	...
	createDepthResources(device, physicalDevice, commandPool, graphicsQueue, swapChainExtent, depthImage, depthImageMemory, depthImageView);
	...
	loadModel(vertices, indexArr);
	createVertexBuffer(device, vertexBuffer, vertexBufferMemory, physicalDevice, commandPool, graphicsQueue, vertices);
	createIndexBuffer(device, physicalDevice, indexBuffer, indexBufferMemory, commandPool, graphicsQueue, indexArr);
	createUniformBuffers(device, physicalDevice, uniformBuffers, uniformBuffersMemory, swapChainImages);
	createDescriptorPool(device, swapChainImages, descriptorPool);
	createDescriptorSets(device, swapChainImages, descriptorPool, descriptorSetLayout, descriptorSets, uniformBuffers, textureImageView, textureSampler);
	createCommandBuffers(commandBuffers, swapChainFramebuffers, commandPool, device, renderPass, swapChainExtent, graphicsPipeline, vertexBuffer, indexBuffer,
		pipelineLayout, descriptorSets, indexArr);
	createSemaphores(device, swapChainImages, imageAvailableSemaphores, renderFinishedSemaphores, inFlightFences, imagesInFlight);
}
...
void cleanupSwapChain(VkDevice device, std::vector<VkFramebuffer> swapChainFramebuffers, VkCommandPool commandPool, std::vector<VkCommandBuffer> commandBuffers, VkPipeline graphicsPipeline,
	VkPipelineLayout pipelineLayout, VkRenderPass renderPass, std::vector<VkImageView> swapChainImageViews, VkSwapchainKHR swapChain, std::vector<VkImage> swapChainImages,
	std::vector<VkBuffer> uniformBuffers, std::vector<VkDeviceMemory> uniformBuffersMemory, VkDescriptorPool descriptorPool, VkImage depthImage, VkDeviceMemory depthImageMemory, VkImageView depthImageView) {
...
void recreateSwapChain(GLFWwindow*& window, VkDevice device, std::vector<VkFence>& imagesInFlight, std::vector<VkImage>& swapChainImages,
	...
	VkDescriptorSetLayout& descriptorSetLayout, std::vector<VkBuffer>& uniformBuffers, std::vector<VkDeviceMemory>& uniformBuffersMemory, VkDescriptorPool& descriptorPool,
	...
	cleanupSwapChain(device, swapChainFramebuffers, commandPool, commandBuffers, graphicsPipeline, pipelineLayout, renderPass, swapChainImageViews, swapChain,
		swapChainImages, uniformBuffers, uniformBuffersMemory, descriptorPool, depthImage, depthImageMemory, depthImageView);
...
void createGraphicsPipeLine(VkDevice device, VkExtent2D swapChainExtent, VkPipelineLayout& pipelineLayout, 
	VkRenderPass renderPass, VkPipeline& graphicsPipeline, VkDescriptorSetLayout descriptorSetLayout) {
...
void createCommandBuffers(std::vector<VkCommandBuffer>& commandBuffers, std::vector<VkFramebuffer> swapChainFramebuffers, VkCommandPool commandPool, 
	VkDevice device, VkRenderPass renderPass, VkExtent2D swapChainExtent, VkPipeline graphicsPipeline, VkBuffer vertexBuffer, VkBuffer indexBuffer,
	...
	for (size_t i = 0; i < commandBuffers.size(); i++) {
		...
		vkCmdDrawIndexed(commandBuffers[i], static_cast<uint32_t>(indexArr.size()), 1, 0, 0, 0);
...
void drawFrame(GLFWwindow* window, VkDevice device, VkQueue graphicsQueue, VkQueue presentQueue, VkSwapchainKHR swapChain, std::vector<VkSemaphore> imageAvailableSemaphores, std::vector<VkSemaphore> renderFinishedSemaphores, std::vector<VkCommandBuffer>& commandBuffers, size_t& currentFrame, std::vector<VkFence> inFlightFences, 
	...
	VkCommandPool& commandPool, VkPipeline& graphicsPipeline, VkPipelineLayout& pipelineLayout, VkBuffer& vertexBuffer, VkBuffer& indexBuffer, bool& framebufferResized, VkDescriptorSetLayout& descriptorSetLayout,
	std::vector<VkBuffer>& uniformBuffers, std::vector<VkDeviceMemory>& uniformBuffersMemory, VkDescriptorPool& descriptorPool, std::vector<VkDescriptorSet>& descriptorSets, VkImageView textureImageView, VkSampler textureSampler, VkImageView depthImageView, VkImage& depthImage, VkDeviceMemory& depthImageMemory,
	...
	if (result == VK_ERROR_OUT_OF_DATE_KHR) {
		recreateSwapChain(window, device, imagesInFlight, swapChainImages, physicalDevice, surface, swapChain, swapChainImageFormat, swapChainExtent,
			swapChainImageViews, renderPass, swapChainFramebuffers, commandPool, commandBuffers, graphicsPipeline, pipelineLayout, vertexBuffer, indexBuffer, descriptorSetLayout,
			uniformBuffers, uniformBuffersMemory, descriptorPool, descriptorSets, textureImageView, textureSampler, depthImageView, depthImage, depthImageMemory, graphicsQueue, indexArr);
		return;
	}
	...
	updateUniformBuffer(device, uniformBuffersMemory, swapChainExtent, imageIndex);
...
/* create vertex buffer and memory allocation */
...
/* creating index buffer - same structure as the vertex buffer */
...
void createIndexBuffer(VkDevice device, VkPhysicalDevice physicalDevice, VkBuffer& indexBuffer, VkDeviceMemory& indexBufferMemory, VkCommandPool commandPool, VkQueue graphicsQueue, std::vector<uint32_t> indexArr) {
	VkDeviceSize bufferSize = sizeof(indexArr[0]) * indexArr.size();
	...
	void* data;
	vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data);
	memcpy(data, indexArr.data(), (size_t)bufferSize);
	vkUnmapMemory(device, stagingBufferMemory);
...
void loadModel(std::vector<Vertex>& vertices, std::vector<uint32_t>& indexArr) {
	...
	for (const auto& shape : shapes) {
		for (const auto& index : shape.mesh.indices) {
			...
			indexArr.push_back(uniqueVertices[vertex]);
		}
	}
}